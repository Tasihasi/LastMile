...
# In DEBUG mode, fall back to an insecure dev key. In production, SECRET_KEY must be set.
...
# Database selection is driven by DATABASE_URL (12-factor):
#   - DEBUG=True (local dev): DATABASE_URL is normally unset, so this falls back
#     to SQLite and local work never touches the production database. CI is also
#     DEBUG=True but sets DATABASE_URL to a throwaway Postgres service, so it
#     still exercises Postgres.
...
# Logging — surface app warnings/errors in gunicorn output
LOGGING = {
    "version": 1,
